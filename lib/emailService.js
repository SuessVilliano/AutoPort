...
async function sendViaResend({ to, cc, bcc, subject, html, text }) {
  ...
  if (!res.ok) {
    ...
    // Don't throw — log the error but don't crash the request
    console.error(`Resend error ${res.status}:`, JSON.stringify(errData));
    return { success: false, provider: 'resend', error: errData.message || `HTTP ${res.status}` };
  }
